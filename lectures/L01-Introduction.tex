...
\titleFrame{
	L01: Introduction to Constraint Solving
}{
	\includegraphics[height=38mm]{images/CO_examples}
}{
	Partly based on slides from Pierre Flener, Guido Tack and Chris Beck.
}
...
\begin{frame}{1. Trial and error on paper}
	You would probably draw the start/end times to get an overview:
	\begin{center}
		\includegraphics[height=0.5\textheight]{images/Skiena_gantt.png}
	\end{center}
	
	and then do some elimination and intuition-driven choices.
	
	$ $
	
	\pause
	Now imagine every movie has a different fee, how to make sure you have the \textbf{optimal} solution?	
\end{frame}

\begin{frame}{2. Do exhaustive search with an algorithm}
	
	\vspace*{-4em} % move content up
	\begin{center}
		\scalebox{0.9}{%
			\begin{tikzpicture}[
				grow=down,
				...
				level 3/.style={sibling distance=8mm}, % smaller spacing for third level
				...
				child { node (L) {}
					child { node (L1) {}
						child { node (L11) {} edge from parent node[draw=none,left=-1px,font=\tiny]{Yes} }
						child { node (L12) {} edge from parent node[draw=none,right=-2px,font=\tiny]{No} }
						edge from parent node[draw=none,left=-1px,font=\tiny]{Yes}
					}
					child { node (L2) {}
						child { node (L21) {} edge from parent node[draw=none,left=-1px,font=\tiny]{Yes} }
						child { node (L22) {} edge from parent node[draw=none,right=-2px,font=\tiny]{No} }
						edge from parent node[draw=none,right=-2px,font=\tiny]{No}
					}
					edge from parent node[draw=none,left=-1px,font=\tiny]{Yes}
				}
				child { node (R) {}
					child { node (R1) {}
						child { node (R11) {} edge from parent node[draw=none,left=-1px,font=\tiny]{Yes} }
						child { node (R12) {} edge from parent node[draw=none,right=-2px,font=\tiny]{No} }
						edge from parent node[draw=none,left=-1px,font=\tiny]{Yes}
					}
					child { node (R2) {}
						child { node (R21) {} edge from parent node[draw=none,left=-1px,font=\tiny]{Yes} }
						child { node (R22) {} edge from parent node[draw=none,right=-2px,font=\tiny]{No} }
						edge from parent node[draw=none,right=-2px,font=\tiny]{No}
					}
					edge from parent node[draw=none,right=-2px,font=\tiny]{No}
				};
				
				% dots directly under each leaf (third layer)
				%\foreach \x in {L11,L12,L21,L22,R11,R12,R21,R22}
				%\node[draw=none,below=6pt of \x] {$\vdots$};
				% small downward-pointing triangles directly under each leaf
				\foreach \x in {L11,L12,L21,L22,R11,R12,R21,R22}
				\draw[line width=0.2pt] 
				(\x.south) ++(0,-0.1) -- ++(-3mm,-12mm) -- ++(6mm,0) -- cycle;
				
				
				% label to the right of the root
				\node[draw=none,right=6pt of root,font=\small] {Tarjan of the Jungle};
				% label to the right of lvl 3
				\node[draw=none,right=6pt of R,font=\small] {The Four Volume Problem};
				% label to the right of lvl 2
				\node[draw=none,right=6pt of R2,font=\small] {The President's Algorist};
				% label to the right of lvl 3
				\node[draw=none,right=6pt of R22,font=\small] {Steiner's Tree};
	...
	\pause
	\begin{itemize}
		\item 9 possible movies $\Rightarrow 2^9 (=512)$ possible solutions to explore
		\item How and when to enforce the non-overlap constraints?
		\item Can search faster by choosing other movie first? What when different fees?
	\end{itemize}
\end{frame}
...
\begin{frame}
	...
	\begin{enumerate}
		\item Trial and error on paper
		\item Write a custom search algorithm
		\item Reuse an existing, generic problem solving paradigm \textbf{$\Leftarrow$ This Course}
	\end{enumerate}
\end{frame}


% originally based on slide from Chris Beck 
\begin{frame}{Model + Solve paradigm}
  \centering
...
\section{Combinatorial Optimisation Problems in the wild}
...
\begin{frame}{Applications in Healthcare}
	\begin{tabular}{CC}
		\structured{Doctor rostering}\invisible{gy} \newline
		\includegraphics[width=50mm]{images/physicians}
		% which doctor for which shift (constraints on availability, preferences, regulations for weekend work etc)
		&
		\structured{Radiotherapy beam optimisation} \newline
		\includegraphics[width=50mm]{images/medical}
		% which beam from which angle with which intensity (constraints on damage to bad cells vs damage to good cells)
		\\[+8mm]
		\structured{Phylogenetic supertree constr.} \newline
		\includegraphics[height=28mm]{images/seabirds}
		% which supertree is maximally consistent with shared species in the tree?
		&
		\structured{Haplotype inference} \newline
		\includegraphics[height=25mm]{images/genotype-haplotype}
		% given n child genotypes, find a minimal set of parent haplotypes so that each given genotype is the merge of 2 found haplotypes
	\end{tabular}
\end{frame}
...
\begin{frame}{Applications in Transportation}
	\begin{tabular}{CC}
		\structured{Waste collection routing} \newline
		% which truck picks up waste where (constraints on capacity, duration, fairness)
		\includegraphics[height=35mm]{images/routes_waste}
		&
		\structured{Container packing} \newline
		% which packages goes where (constraints on volume, stability)
		\includegraphics[height=32mm]{images/binPacking}
		\\[+8mm]
		...
		\end{tabular}
		&
		\structured{Traffic control sectorisation} \newline
		% which traffic controller manages which space (constraints on contiguity, balance)
		\includegraphics[height=25mm]{images/sectorLondon}    
	\end{tabular}
\end{frame}

%TODO Tias: add Flanders Make example in style of Doctor Rost above?
\begin{frame}
...
\begin{frame}{Applications in Manufacturing}
  \begin{tabular}{CC}
  	\structured{Transporting aircraft parts} \newline
  	\includegraphics[height=30mm]{images/gantt}
    &
    \structured{Timetabling} \newline
    \includegraphics[height=30mm]{images/timetable}
    \\[+8mm]
    \structured{Robot programming} \newline
    \includegraphics[height=20mm]{images/ABB-FRIDA}
    &
    \structured{Base-station testing}
    \includegraphics[height=20mm]{images/baseStation}
  \end{tabular}
\end{frame}

\begin{frame}
	
	\alert{Combinatorial optimization problems are everywhere!} \vfill
	
	\defined{Combinatorial} = finding the best \textit{combination} of objects from a finite set of possibilities.\vfill
		
	We typically categorize problems by their \textbf{structural nature}, for example:
	{\small
	\begin{itemize}\setlength{\itemsep}{0em} % extra vertical space between items
		\item \defined{Assignment problems:} assign objects to fixed slots \\
		      \textit{(doctor rostering, haplotype inference, base-station testing)}
		\item \defined{Scheduling problems:} assign objects freely in time \\
		      \textit{(transport scheduling, robot programming)}
		\item \defined{Packing problems:} assign objects in 2D or 3D space \\
		      \textit{(bin packing, container packing, carpet cutting)}
		\item \defined{Routing problems:} enforce graph properties like cycles, connectivity \\
		      \textit{(vehicle routing, waste collection, traffic control sectorisation)}
	\end{itemize}
    }
\end{frame}


\section{Why are these problems hard?}
...
\begin{frame}
	...
	Could you find a solution for 50 movies? For 1000s of movies across 100 actors?
\end{frame}


\begin{frame}{Finding solutions with exhaustive search}
	
	\vspace*{-1em} % move content up
	\begin{center}
		\scalebox{0.5}{%
			\begin{tikzpicture}[
				grow=down,
	...
	\begin{itemize}
		\item input = 50 actors and 1000 movies, is $2^{50*1000}$ possible solutions...
		\item Search requires a worst-case \textbf{exponential} number of options to explore
	\end{itemize}\vfill
	
	\defined{Exponential:} running time grows exponentially with input size $n$, \\
	\qquad $T(n) \sim c^{n}$ for some constant $c > 1$; \qquad big-oh notation: $\Oh{c^\mathbf{n}}$
	
	\defined{Polynomial:} running time is bounded by a polynomial function of input size $n$, \\
	\qquad $T(n) \leq c*n^x$ for some constant $c > 0$ and $x \geq 0$; \qquad big-oh notation: $\Oh{\mathbf{n}^x}$ \vfill
	
	Does there exist a \textbf{polynomial} algorithm for combinatorial optimisation problems?
	
\end{frame}


\begin{frame}{Limits of computation: P $\overset{?}{=}$ NP \hfill (Cook, 1971; Levin, 1973)}
  Informally:
  \begin{itemize}
    ...
    \item P = class of problems with easy-to-\alert{compute} solutions (polynomial time) \\
      NP = class of problems with easy-to-\alert{check} solutions  (polynomial \textit{check})
  \end{itemize}
...
\begin{frame}{Examples of NP problems}
  ...
  \begin{examples}
  	\begin{itemize}
  		...
  		(which visits each \emph{vertex} once) is in NP (best known: $\Oh{1.657^n}$).
  	\end{itemize}
  \end{examples}
  \begin{examples}
    \begin{itemize}
    ...
    \item Determining the existence of a simple path (which has
      ...
      \emph{at least} a given number $\ell$ of edges is NP.  
      Hence finding a \defined{longest path}
...
\iffalse 'refined' rather hand-wavy... skip for now
\begin{frame}{What vs How}
  ...
  An \defined{algorithm} is an \textbf{imperative} description of \textbf{how} to solve the problem.
\end{frame}
\fi
...
\begin{frame}
	...
	and / or \vfill
	\begin{itemize}
		\item[how:] \textbf{Solving} combinatorial problems \textbf{intelligently}: \vfill
		\begin{itemize}
...
\begin{frame}
	\begin{examples}[Declarative problem solving paradigms]
		...
		\end{itemize}
	\end{examples}
	\begin{examples}[Non-declarative solving paradigms]
		Methodologies (e.g. typically without separated concept of 'model' and 'solver'):
		\begin{itemize}
...
\begin{frame}[fragile]
  ...
  \begin{example}[Sudoku CP model]
    ...
    \begin{align*}
      ...
      &\qquad \qquad \qquad \quad ~G_{p+1, q}, G_{p+1, q+1}, G_{p+1, q+2}, && \notag \\
      &\qquad \qquad \qquad \quad ~G_{p+2, q}, G_{p+2, q+1}, G_{p+2, q+2}), \quad &&\forall p, q \in \{1, 4, 7\} \\ 
...
\begin{frame}{Fun with sudoku's}
	Sudoku Assistant: \url{https://visualsudoku.cs.kuleuven.be}
	\vfill
	
	\begin{center}
		\includegraphics[height=14em]{images/sudoku_assistant}
	\end{center}
	\vspace{-1em}
	
	Solving is instant and generating hints is real-time, both use CP solvers. \\
	The ML predictions (vision transformer) are the slowest.
\end{frame}
...
\begin{frame}{Parameterized Constraint Models}
	
	A model, e.g. a constraint specification, is typically written down as a parameterized constraint model.
	\vfill
	
	\begin{itemize}
		\item A \defined{parameterized constraint model} has uninstantiated parameters. 
		For example the number of grains and plots in the agricultural experiment design, or the given cells hints in a sudoku. \vfill
		
		\item The parameterized constraint model represents a \textit{class} of problems.\vfill
		
		\item The uninstantiated parameters correspond to the input \defined{data}.\vfill
...
\begin{frame}
	...
	\textbf{Instance}: $7$ \Block s, $7$ \Variety s, $3$ \Variety
	s/\Block, $3$ \Block s/\Variety, balance~$1$. \vfill
	
	General problem: \defined{balanced incomplete block design} (\defined{BIBD})
\end{frame}
...
\begin{frame}
  ...
  \begin{example}[BIBD \emph{integer} model in CPMpy]
    \lstinputlisting[language=cpmpy,firstline=4,lastline=18]{models_cpmpy/T01_bibd.py}
  \end{example}  
\end{frame}
\end{flashcardcpmpy}
...
\begin{frame}
  ...
  \begin{example}[BIBD \emph{set} CP model]
    ...
    \begin{align*}
      ...
      &\sum_{i=1}^v \Iverson{j \in \mathcal{B}_i} = r, \quad &&\forall j \in \{1, 2, \dots, b\}, \, \mbox{Each block contains exactly block-size varieties} \\
  ...
  \textit{Notation:} $\Iverson{C}$ is the Iverson bracket: $1$ if condition $C$ holds, and $0$ otherwise.
...
\begin{frame}
  \begin{example}[Doctor Shift Scheduling CP Model]
    ...
    \vspace{-1em}
    \begin{align*}
      &R_{pd} \in \{\text{Appt}, \text{Call}, \text{Oper}, \text{Free} \}, \quad &&\text{\tiny $\forall p \in \{1, \dots, {\text{n\_doctors}}\}, \, d \in \{1, \dots, {\text{n\_days}}\}$} \\
      %\\
      %&\text{where } R_{pd} = \text{Shift type for doctor } p \text{ on day } d, \\
      &\text{1. \#on-call doctors / day = 1: } 
      \sum_{p=1}^{{\text{n\_doctors}}} \Iverson{R_{pd} = \text{Call}} = 1, \quad &&\text{\tiny $\forall d \in \{1, \dots, {\text{n\_days}}\}$} \\
      &\text{2. \#operating doctors / weekday $<= 2$: } 
      \sum_{p=1}^{{\text{n\_doctors}}} \Iverson{R_{pd} = \text{Oper}} \leq 2, \quad &&\text{\tiny $\forall d \in \{1, \dots, {\text{n\_days}}\}, \, \text{if } d\mod 7 \in 1..5$} \\
      &\text{3. \#operating doctors / week $\geq 7$: } 
      \sum_{p=1}^{{\text{n\_doctors}}}~\sum_{d=s}^{s+6} \Iverson{R_{pd} = \text{Oper}} \geq 7, \quad &&\text{\tiny $\forall s \in \{1, \dots, {\text{n\_days}}\}, \, \text{if } s\mod 7 = 1$} \\
      &\text{4. \#appointed doctors / week $\geq 4$: }
      \sum_{p=1}^{{\text{n\_doctors}}}~\sum_{d=s}^{s+6} \Iverson{R_{pd} = \text{Appt}} \geq 4, \quad &&\text{\tiny $\forall s \in \{1, \dots, {\text{n\_days}}\}, \, \text{if } s\mod 7 = 1$} \\
      &\text{5. day off after operation: } 
      (R_{pd} = \text{Oper}) \rightarrow (R_{p,d+1} = \text{Free}), \quad &&\text{\tiny $\forall p \in \{1, \dots, {\text{n\_doctors}}\}, \, d \in \{1, \dots, {\text{n\_days}}-1\}$} 
      %&\text{6. Maximize nr free shifts on weekends:} \\
      %&\max \sum_{s:\, s\mod 7 = 1} \sum_{p=1}^{n_{\text{doctors}}} \sum_{d \in \{s+5,s+6\}} \Iverson{R_{pd} = \text{Free}} \\
    \end{align*}
  \end{example}
  \textit{Notation:} $\Iverson{C}$ is the Iverson bracket: $1$ if condition $C$ holds, and $0$ otherwise.
\end{frame}
...
\begin{frame}
  \begin{example}[Job allocation at minimal salary cost]
    \structured{Given} $n\_jobs$ jobs, $n\_appls$ applicants and the salaries of work applicants $salary$, \\ 
    \structured{Find} a work applicant for each job \\ 
    \structured{Such that} each applicant does at most $k$ jobs\\ 
    ~~ and the total salary cost is minimal: 

    \small
    \vspace{-1em}
    \begin{align*}
      &\text{salary}_a = \textit{...given value...} \quad && \forall a \in \{1, \ldots, \text{n\_appls}\} \\
      & &&\\
      &W_j \in \{1, \dots, {\text{n\_appls}}\}, \quad &&\forall j \in \{1, \dots, {\text{n\_jobs}}\} \\
      %&\quad \text{where } w_j \text{ represents the applicant assigned to job } j. \\
      & \sum_{j=1}^{n\_jobs} \Iverson{W_j = a} \leq k && \forall a \in \{1, \ldots, \text{n\_appls}\} \\
      & &&\\
...
\begin{frame}
  \begin{example}[Job allocation at minimal salary cost]
    ...
    \lstinputlisting[language=cpmpy,firstline=11,lastline=20]{models_cpmpy/T01_jobAlloc.py}
    \vspace{-3mm}
  \end{example} \vfill

  \textcolor{Melon}{Special power} of constraint programming languages: discrete integers: \\
  ~~~~ Using a decision variable (\texttt{worker[j]}) to index into an array (\texttt{salary[]}) \\
  ~~~~ \textit{Internally: will translate to an 'Element' global constraint}
\end{frame}
\end{flashcardcpmpy}
\begin{flashcardminizinc}
\begin{frame}[fragile]
  ...
  \begin{example}[Traveling Salesperson Problem]
    ...
    \begin{minipage}[c]{0.6\textwidth}
      ...
      \begin{align*}
        ...
        \end{bmatrix} \\
        &n\_cities = 4 \\
        &Next_c \in \{0, \dots, n\_cities-1\}, \quad \text{\tiny $\forall c \in \{0, \dots, n\_cities-1\}$} \\
        &\minimize \sum_{c=0}^{{\text{n\_cities}}-1} \text{distance}[c,Next_c] \\
        \temporal<2>{}
...
\begin{flashcardcpmpy}
\begin{frame}
  \begin{example}[Traveling Salesperson Problem (indexing offset 0)]
...
\begin{frame}{Decision Variables, Parameters, and Identifiers} 
  \begin{itemize}
    \item Parameters and decision variables in a model are concepts different from programming variables in an imperative or object-oriented program. \vfill
    ...
    \item A \defined{decision variable} in a model is like a variable in mathematics: \\
          it is \emph{not} given a value in a model, and \\
          its value is only fixed in a solution (if a solution exists). \vfill
          
    \item A parameter or decision variable is referred to by an \defined{identifier} (a name like \texttt{Given}, \texttt{Grid}, \texttt{Next}, ...). \vfill
    
    % 'index identifier' is a bit of a weird name, should be clear what quantification does?
...
\begin{frame}{Modelling Concepts (end)}
  \begin{itemize}
    ...
    \begin{itemize}
      ...
      \item find an optimal solution
      \item find all optimal solutions
...
\begin{flashcardcpmpy}
	\begin{frame}{Constraint-Based Modelling}
		...
		\emph{library} (\alert{not a solver}): \vfill
		\begin{itemize}
		...
		Compared to the library of a specific solver (e.g. \textit{ortools} or \textit{gurobi}), you can specify problems at a higher level, e.g. nested expressions, and use globals that the solver might not support.
		
		CPMpy will \textit{transform} this to a lower-level solver library for you.
	\end{frame}
\end{flashcardcpmpy}
\begin{flashcardminizinc}
	\begin{frame}{Constraint-Based Modelling}
...
\begin{frame}{Solving a model}
  ...
  {\small
  \begin{itemize}
  	\item \textbf{SMT:} Bool\&Int\&String\&... decision variables; logical, theory-specific constraints
  	\item \textbf{CP:} Bool\&Int decision variables; logical, mathematical, global constraints
  	\item \textbf{MIP:} Integer(ILP)\&Floating-point(MIP) decision variables; linear constraints \& objective
  	%\item \textbf{LP:} Floating-point decision variables; linear constraints \&
  	\item \textbf{PB:} Boolean decision variables; linear (Pseudo-Boolean) constraints \& objective
    \item \textbf{SAT:} Boolean decision variables; clause constraints. MaxSAT: linear objective
  \end{itemize}
  }
\end{frame}
...
\begin{frame}{There Are Many Solving Technologies}
  ...
  \item Some technologies have \alert{non-standardised} modelling languages \\ 
        across their solvers: CP and LCG \textit{(although: FlatZinc, XCSP3)} \vfill
  ...
  No technology universally dominates all the others \vfill
  
  One should test several technologies on each problem! \\
  \textit{(easy in solver-independent modelling frameworks)}
\end{frame}
...
\begin{frame}{How to Solve a Combinatorial Optimisation Problem?}
  ...
  \alt<2->{Not so easy, but easier than implementing combinatorial algorithms from scratch!\\ (... where you would have to consider most modelling decisions either way)}{Easy, right?}
\end{frame}

\begin{frame}{Key take-aways, Model and Solve:}
...
\begin{frame}{Lecture Plan}
	...
	\begin{itemize} \setlength{\itemsep}{0pt}
		...
		\item L08: Portfolio solvers, algorithm configuration and parallelism
	\end{itemize}
